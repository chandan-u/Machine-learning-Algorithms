...
# def crossentropy(Xtrain, ytrain, weights):
...
#         sigma =  1/( 1 + np.exp( np.negative(np.dot(x, weights) ) ) )
#         summation.append(y - sigma)

#     return np.linalg.norm(summation)

def cost_diff( network_output, target):
...
def proximalOperator(w, param, eeta):
    c = param * eeta
